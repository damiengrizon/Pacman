...
class Player(pygame.sprite.Sprite):
    ...
    def __init__(self, x, y, filename):
        ...
        # crée les objet pour les animations
        self.move_rigth_animation = Animation(img, 32, 32)
    ...
    def update(self, horizontal_blocks, vertical_blocks):
        if not self.explosion:
            ...
            # Ceci empêchera l'utilisateur de monter ou de descendre lorsqu'il se trouve à l'interieur du cadre
            ...
            for block in pygame.sprite.spritecollide(self, vertical_blocks, False):
                ...
                self.change_x = 0
            # Ceci entrainera le démarage de l'animation
            if self.change_x > 0:
                self.move_rigth_animation.update(10)
                self.image = self.move_rigth_animation.get_current_image()
...
class Animation(object):
    def __init__(self, img, width, height):
        # charge les feuilles de sprites du jeu
        self.sprite_sheet = img
        # crée une liste pour stocké les images
        self.image_list = []
        self.load_images(width, height)
        # crée une variable qui contiendra l'image de la liste
        self.index = 0
        # crée une variable qui contiendra le temps de jeu
        self.clock = 1

    def load_images(self, width, height):
        # parcourt chaque image de la feuille de sprites
    ...
    def get_image(self, x, y, width, height):
        ...
        # copie le sprite de la grande feuille sur la plus petite
        image.blit(self.sprite_sheet, (0, 0), (x, y, width, height))
        # le noir fonctionne comme la couleur transparente
    ...
    def update(self, fps=30):
        step = 30 // fps
        temp = range(1, 30, step)
        if self.clock == 30:
            self.clock = 1
        else:
            self.clock += 1

        if self.clock in temp:
            # increase index
            self.index += 1
